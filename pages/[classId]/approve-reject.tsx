...
const ApproveReject: React.FC<{ students: Student[]; classId: string }> = (
    props
) => {
    ...
    return (
        <Fragment>
            <Head>
                ...
                <meta
                    name="description"
                    content="Approve/Reject attendance platform."
                ></meta>
            </Head>
            <StudentList
...
export const getServerSideProps = withSession(async (context) => {
    ...
    const studentIds = students.map(
        (student) => new Object(new ObjectId(student.id))
    );
    const usersCollection = db.collection("users");
    const users = await usersCollection
        .find({ _id: { $nin: studentIds }, role: "student" })
        .toArray();
    users.forEach((user) => {
        students.push({
